...
contract Halo2VerifyingKey {
    constructor() {
        assembly {
            ...
            {%- let offset = constants.len() + 2 * (fixed_comms.len() + permutation_comms.len() + num_advices_user_challenges.len()) + const_expressions.len() + 2 %}
            mstore({{ (32 * (offset + loop.index0))|hex_padded(4) }}, {{ gate_computations_len|hex_padded(64) }}) // gate_computations_len[{{ loop.index0 }}]
            {%- endfor %}
            return(0, {{ (32 * (constants.len() + 2 * (fixed_comms.len() + permutation_comms.len() + num_advices_user_challenges.len()) + const_expressions.len() + 2 + gate_computations_lens.len()))|hex() }})
        }
    }
}
